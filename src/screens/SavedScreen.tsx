  ...
  return (
    <View style={{ flex: 1, backgroundColor: C.bg }}>
      <View style={{ paddingTop: insets.top + 8, paddingBottom: 14, paddingHorizontal: 20, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
        ...
        <Text style={{ fontFamily: F.serif, fontSize: 26, letterSpacing: -0.6, color: C.ink }}>Saved spots</Text>
      </View>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 40 }}>
        {saved.length === 0 ? (
          <View style={{ alignItems: 'center', paddingTop: 50 }}>
            ...
            <Text style={{ fontFamily: F.serif, fontSize: 24, marginTop: 20, color: C.ink, textAlign: 'center' }}>No saved spots yet.</Text>
            ...
            {saved.map((s) => (
              <Pressable key={s.id} onPress={() => open(s)} style={[{ flexDirection: 'row', gap: 12, alignItems: 'center', backgroundColor: C.surface, borderRadius: R.lg, padding: 12 }, SH.card]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text numberOfLines={1} style={{ fontSize: 12.5, color: C.ink3, fontFamily: F.regular, marginTop: 1 }}>{s.category || 'Saved'}{s.city ? ` · ${s.city}` : ''}</Text>
                </View>
