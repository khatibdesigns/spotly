    ...
    <View style={{ flex: 1 }}>
      ...
      <View style={{ position: 'absolute', left: 0, right: 0, bottom: 200, paddingHorizontal: 28, alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 16, lineHeight: 23, color: C.ink2, fontFamily: F.regular, textAlign: 'center', marginTop: 16, maxWidth: 290 }}>
          Find places your kids will love, plan the day, and keep every memory in one warm place.
        </Text>
      </View>
      <View style={{ position: 'absolute', left: 24, right: 24, bottom: insetsBottom + 24, gap: 10 }}>
        <Btn kind="primary" size="lg" full onPress={onStart}>Get started</Btn>
        <Btn kind="ghost" size="lg" full onPress={onSignIn}>I already have an account</Btn>
      </View>
    </View>
  );
}
...
function Pillars({ insetsTop }: { insetsTop: number }) {
  const pillars = [
    { ic: Icons.compass, c: C.coral, t: 'Find places', d: 'A curated feed of kid-friendly spots tuned to where you are.' },
    { ic: Icons.calendar, c: C.sky, t: 'Make the day happen', d: 'Plan, book, and get reminders before you head out.' },
    { ic: Icons.album, c: C.sage, t: 'Keep the memories', d: 'Pins, photos, and printable albums you’ll actually keep.' },
  ];
  return (
    <ScrollView contentContainerStyle={{ paddingTop: insetsTop + 30, paddingHorizontal: 28, paddingBottom: 160 }}>
      <Text style={{ fontFamily: F.mono, fontSize: 13, color: C.ink3, letterSpacing: 1, textTransform: 'uppercase' }}>How Spotly works</Text>
